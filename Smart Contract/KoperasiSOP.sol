...
contract KoperasiSimpanPinjam is ReentrancyGuard, Ownable {
    ...
    struct Pinjaman {
        ...
        bool isRestructured;
        string surveyNote;           // Added
        address approvedByCommittee; // Added
    }
    ...
    mapping(address => uint256) public tagihanWajib; // Added: JDCOOP Billing
    ...
    event MemberProfileUpdated(address indexed member, string field, uint256 timestamp); // Added
    event TagihanDibuat(uint256 nominalTotal, uint256 timestamp); // Added
    event BagiHasilDirilis(uint256 nominalTotal, uint256 timestamp); // Added
    ...
    function recordDeposit(address _user, uint256 _amount, bool _isWajib) external hanyaPengurus openPeriod {
        ...
        // Increase liquidity pool on deposit
        settings.currentLiquidityPool += _amount;

        emit DepositTercatat(_user, _amount, _isWajib ? "Wajib" : "Sukarela", block.timestamp);
    }

    function recordWithdrawal(address _user, uint256 _amount) external hanyaPengurus openPeriod {
    ...
    // --- JDCOOP ADVANCED ACCOUNTING ---

    /**
     * @dev Generate monthly bills for mandatory savings (Tagihan Simpanan Wajib)
     */
    ...
    /**
     * @dev Member pays their outstanding mandatory savings bill
     */
    function bayarTagihanWajib(uint256 _amount) external hanyaAnggota openPeriod {
    ...
    /**
     * @dev Release Service/Interest to all members from Profit Pool
     */
    function rilisBagiHasil(uint256 _percentage) external hanyaPengurus {
        require(_percentage <= 100, "Max 100%");
        uint256 amountToDistribute = (profitBelumDibagi * _percentage) / 100;
        require(amountToDistribute > 0, "No profit to share");

        for (uint i = 0; i < listAlamatAnggota.length; i++) {
            address member = listAlamatAnggota[i];
            if (dataAnggota[member].status == MemberStatus.Active) {
                uint256 bal = idrToken.balanceOf(member);
                if (bal > 0 && totalSimpananSeluruhAnggota > 0) {
    ...
    function ajukanPinjaman(uint256 _amount, uint256 _tenorBulan) external hanyaAnggota openPeriod {
        ...
        // --- SOLUSI STACK TOO DEEP ---
        // Kita panggil tempat penyimpanannya dulu, lalu isi datanya satu per satu ke bawah.
    ...
    function setujuiPinjaman(uint256 _loanId) external hanyaPengurus openPeriod {
        Pinjaman storage p = dataPinjaman[_loanId];
        require(p.status == StatusPinjaman.CommitteeApproved, "Harus melalui tahap Komite");

        // JDCOOP: Calculate Fees
        ...
        if (settings.deductFeesUpfront) {
            // JDCOOP Style: Deduct from disbursement
        ...
        // Disburse tokens (already reduced if deductFeesUpfront is true)
    ...
    function recordAngsuran(uint256 _loanId, uint256 _amount) external hanyaPengurus openPeriod {
        Pinjaman storage p = dataPinjaman[_loanId];
        ...
        if (block.timestamp > p.waktuJatuhTempo) {
            ...
            if (overdueDays > 0) {
                ...
                // Penalty goes to Profit Pool first
                if (_amount > denda) {
        ...
        if(loanBalance[p.peminjam] >= amountToPrincipal) {
            loanBalance[p.peminjam] -= amountToPrincipal; 
        } else {
        ...
        if (p.sudahDibayar >= p.totalHarusDibayar) {
            ...
            uint256 interestEarned = (p.totalHarusDibayar > p.jumlahPinjaman) ? (p.totalHarusDibayar - p.jumlahPinjaman) : 0;
            profitBelumDibagi += interestEarned;
    ...
    function openSimpananBerjangka(uint256 _amount, uint256 _tenorBulan) external hanyaAnggota {
        require(idrToken.balanceOf(msg.sender) >= _amount, "Saldo tidak cukup");
        
        // Lock percentage usually higher (e.g. 1% per month)
        uint256 rate = _tenorBulan * 1; 
    ...
    function tutupKeanggotaan(address _member) external hanyaPengurus {
        require(dataAnggota[_member].terdaftar, "Anggota tidak ditemukan");
        require(loanBalance[_member] == 0, "Masih memiliki pinjaman aktif");
        require(idPinjamanAktifAnggota[_member] == 0, "Ada pengajuan pinjaman");
